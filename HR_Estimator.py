...
class GUI(QMainWindow, QThread):
    def __init__(self):
        ...
        self.input_rgb_image = Image_RGB()
    ...
    def selectInput(self):
        ...
        if self.cbbInput.currentIndex() == 0:
            ...
            self.btnOpen.setEnabled(True)
            self.btnOpen.setText("Open \n Image folder ")
            self.statusBar.showMessage("Input: Image", 5000)
    ...
    def openFileDialog(self):
        ...
        if self.cbbInput.currentIndex() != 2:
            ...
            self.dirname = QFileDialog.getExistingDirectory(self, "QFileDialog.getOpenFileName()", "", options=options)
    ...
    def selectOpenInput(self):
        if self.cbbInput.currentIndex() == 1:
            ...
            print("open camera")
            self.open_camera()
        elif self.cbbInput.currentIndex() == 0:
            if self.cbbMode.currentIndex() == 1:
            ...
            self.input = self.input_rgb_image
    ...
    def estimate_single_bpm(self):
        if not self.status:
            ...
            while self.status:
                ...
                self.signal_Plt.clear()
                #self.signal_Plt.plot(self.process.RGB_signal_buffer[1], pen='r')  # Plot green signal
                self.signal_Plt.plot(self.process.FREQUENCY[:720], self.process.PSD[:720], pen='r')  # Plot fused PSD

                self.fft_Plt.clear()
                #self.fft_Plt.plot(self.process.FREQUENCY[:300], self.process.PSD[:300], pen='r')  # Plot fused PSD
                self.fft_Plt.plot(self.process.RGB_signal_buffer[1], pen='r')  # Plot green signal

                self.trend_Plt.clear()
                self.trend_Plt.plot(self.process.test4, pen='r')  # Plot NIR's PSD

                self.test1_Plt.clear()
                self.test1_Plt.plot(self.process.FREQUENCY[:720], self.process.test1[:720],
                                    pen='r')  # Plot each component's PSD

                self.test2_Plt.clear()
                self.test2_Plt.plot(self.process.FREQUENCY[:720], self.process.test2[:720],
                                    pen='r')  # Plot each component's PSD

                self.test3_Plt.clear()
                self.test3_Plt.plot(self.process.FREQUENCY[:720], self.process.test3[:720], pen='r')
                self.running = False
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.process.set_process_mode(self.mode)
