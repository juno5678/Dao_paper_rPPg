...
#######################################################################
'''
This code has been remodeled and updated with latest methods from the latest numpy version 1.18.1 by Le Quang Dao on 12/20/2019
I own no copyright for this file since it was initially distributed free for public
'''
import numpy as np
...
def jadeR(X, m=None, verbose=True):
    ...
    assert isinstance(X, np.ndarray), "X (input data matrix) is of the wrong type (%s)" % type(X)
    orig_type = X.dtype  # remember to return matrix B of the same type
    X = np.array(X.astype(np.float64))
    assert X.ndim == 2, "X has %d dimensions, should be 2" % X.ndim
    assert (verbose is True) or (verbose is False), "verbose parameter should be either True or False"

    [n, T] = X.shape  # GB: n is number of input signals, T is number of samples

    if m is None:
        m = n  # Number of sources defaults to # of sensors
    assert m <= n, "jade -> Do not ask more sources (%d) than sensors (%d )here!!!" % (m, n)
    ...
    [D, U] = lna.eig(np.matmul(X, X.T) / float(T))  # An eigen basis for the sample covariance matrix
    k = D.argsort()
    ...
    B = np.matmul(np.diag(1./scales), B)  # Now, B does PCA followed by a rescaling = sphering
    # B[-1,:] = -B[-1,:] # GB: to make it compatible with octave
    # --- Sphering ------------------------------------------------------
    X = np.matmul(B, X)  # %% We have done the easy part: B is a whitening matrix and X is white.
    ...
    CM = np.zeros((m, int(m*nbcm)))  # Storage for cumulant matrices
    ...
    for im in range(m):
        ...
        Qij = np.matmul(np.multiply(Xijm[:, None], X).T, X) / float(T) - R - 2 * np.matmul(R[:, im], R[:, im].T)
        CM[:, Range] = Qij
        Range = Range + m
        for jm in range(im):
            Xijm = np.multiply(Xim, X[:, jm])
            Qij = np.sqrt(2) * (np.matmul(np.multiply(Xijm[:, None], X).T, X) / float(T) -
                                np.matmul(R[:, im], R[:, jm].T) - np.matmul(R[:, jm], R[:, im].T))
            CM[:, Range] = Qij
            Range = Range + m
    ...
    for im in range(nbcm):
        ...
        On = On + (np.multiply(Diag, Diag)).sum(axis=0)
        Range = Range + m
    ...
    A = lna.pinv(B)
    keys = np.argsort(np.multiply(A, A).sum(axis=0))
    B = B[keys, :]
    B = B[::-1, :]  # % Is this smart ?
    ...
    B = np.matmul(np.diag(signs), B)
    return B.astype(orig_type)
